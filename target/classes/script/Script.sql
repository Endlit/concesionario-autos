...
INSERT INTO usuario(dni, nombre, apellido, rol, correo)
VALUES(?, ?, ?, ?, ?)
...
-- Si *trae clienteDni*
UPDATE vehiculo SET
                    marca = ?, modelo = ?, precio = ?, cliente_dni = ?
WHERE matricula = ?;

-- Si *NO trae cliente (null)*
UPDATE vehiculo SET
                    marca = ?, modelo = ?, precio = ?, cliente_dni = NULL
WHERE matricula = ?;

-- Nuevo registro (si no existe)
INSERT INTO vehiculo(matricula, marca, modelo, precio, cliente_dni)
VALUES (?, ?, ?, ?, ?)
    ON DUPLICATE KEY UPDATE
                         marca = VALUES(marca),
...
-- 1. Insertar venta
INSERT INTO venta (fecha, precio_venta, vendedor_dni, vehiculo_matricula)
VALUES (CURDATE(), ?, ?, ?);
...
INSERT INTO venta_opcion (venta_id, opcion_id, precio_aplicado)
